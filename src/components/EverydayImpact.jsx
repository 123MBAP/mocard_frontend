...
import { GraduationCap, Receipt, Smartphone, Landmark, HeartPulse, ArrowRight } from 'lucide-react';

export default function EverydayImpact() {
  const useCases = [
    {
      icon: <GraduationCap className="w-6 h-6 text-pp-blue" />,
  ...
  return (
    <section className="bg-slate-50 py-20 md:py-28 border-t border-slate-200/80 relative overflow-hidden scroll-mt-24" id="everyday-impact">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Section Header */}
        <div className="max-w-3xl mx-auto text-center mb-20 space-y-4 group cursor-pointer">
          ...
          <p className="text-slate-650 font-sans text-base md:text-lg font-light">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {useCases.map((useCase, idx) => (
            <div 
              key={idx}
              className="group bg-white border border-slate-200 rounded-3xl p-6 hover:border-slate-350 hover:bg-white hover:shadow-xl transition-all duration-300 flex flex-col justify-between cursor-pointer"
            >
              <div className="space-y-6">
                {/* Icon and Tag Badge */}
                <div className="flex justify-between items-center">
                  <div className="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-100 group-hover:border-pp-blue/10 group-hover:bg-pp-blue/5 text-slate-700 flex items-center justify-center transition-all duration-300">
                    {useCase.icon}
                  </div>
                  <span className="text-[10px] font-bold bg-slate-100 text-slate-600 px-3 py-1 rounded-full uppercase tracking-wider font-outfit">
                    {useCase.badge}
                  </span>
                </div>
          ...
          <div className="group bg-pp-black text-white border border-slate-800 rounded-3xl p-6 flex flex-col justify-between hover:shadow-xl transition-all duration-300 relative overflow-hidden cursor-pointer">
            {/* Subtle light glow */}
            <div className="absolute top-0 right-0 w-48 h-48 rounded-full bg-pp-blue/10 blur-2xl pointer-events-none" />
            
            <div className="space-y-6 relative z-10">
              <span className="text-[10px] font-bold bg-slate-900 border border-slate-800 text-pp-blue px-3 py-1 rounded-full uppercase tracking-wider font-outfit">
                Developer Sandbox
              </span>
              
              <div className="space-y-2 pt-2">
                <h3 className="text-2xl font-bold font-outfit leading-tight">Need a custom payment utility?</h3>
